...
#                    self.pool.get('account.payments').unlink(cr, uid, [pay.check_id.id], context)
    ...
    def pay_provision(self, cr, uid, ids, context=None):
        ...
        for pay in self.browse(cr, uid, ids, context):
            ...
            for line in pay.line_ids:
                if (not pay.provision_id.pay_prov and line.pay_aprove) or (pay.provision_id.pay_prov):
                    if line.amount_total:
                        ...
                        total_amount = line.total_amount_provision                    
                        amount+=line.total_amount_provision
                    move_line_pool.create(cr, uid, {
                            'name': ref or '/',
                            'debit': 0,
                    ...
                    if line.mode_id.check: 
                        cheque_id = self.pool.get('check.receipt').search(cr, uid,[('state','=','open')])[0] or False  
                        ...
                        prov_line_obj.write(cr, uid, [line.id], {'check_id':check_id})    
                else:
                    lines_ids.append(line.id)
            if amount>=0:
                    move_line2=move_line_pool.create(cr, uid, {
            ...
            y_from = datetime.strptime(pay.date_from[0:10],"%Y-%m-%d")
            y_to = datetime.strptime(pay.date_to[0:10],"%Y-%m-%d")
            y_from = y_from.year
            y_to = y_to.year
            self.pool.get('hr.provision').change_date(cr,uid,ids,pay.provision_id.id,y_from,y_to,context)
