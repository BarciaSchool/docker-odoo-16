...
class stock_invoice_incoming(osv.osv_memory):
    ...
    def onchange_number_in(self, cr, uid, ids, number, type=None, address=None, journal=None, origin='local', date=None, printer_id=None,
                           shop_id=None, electronic=False, context=None):
        if context is None:
            context = {}
        result = self.pool.get('account.invoice').onchange_number_in(cr, uid, ids, number, 'in_invoice', address, journal, origin, date, printer_id,
                                                                     shop_id, electronic, context=context)
        return result
