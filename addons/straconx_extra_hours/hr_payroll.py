...
class hr_aa_payroll(osv.osv):
    ...
    def build(self, cr, uid, date_from, date_to, emp_ids=[], name_payroll=None, context=None):
        pool_emp = self.pool.get('hr.employee')
        pool_line = self.pool.get('hr.aa.payroll.line')
        pool_con = self.pool.get('hr.contract')

	if name_payroll:
	        pr_name = "%s - %s - %s" % (date_from, date_to, name_payroll)
	else:
    ...
    def compute(self, cr, uid, ids, context=None):
        lines_to_compute = []
	# Cambios Gustavo
	# Se agrega emp_ids para levantar el employee id del payroll
        pool_line = self.pool.get('hr.aa.payroll.line')

	args = [('payroll_id','=',ids)]
	line_ids = pool_line.search(cr,uid,args,context=context)
        lines = pool_line.browse(cr, uid, line_ids)
	emp_list = []
	for line in lines:
...
class hr_aa_payroll_value(osv.osv):
	...
	cache = {}
	    ...
	    # import pdb;pdb.set_trace()
            if False and value.formula_id.name == 'Horas Fin de Semana':
               import pdb;
               pdb.set_trace()
