...
class crossovered_budget_lines(osv.osv):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            if line.analytic_account_id.id:
                cr.execute("SELECT SUM(debit-credit) FROM account_move_line WHERE analytic_account_id=%s AND (date "
                           ...
                           (line.analytic_account_id.id, date_from, date_to, line.department_id.id, line.shop_id.id))
    ...
    _columns = {
        ...
        'crossovered_budget_id': fields.related('general_budget_id', 'crossovered_budget_id', type='many2one', relation='crossovered.budget',
                                                string='Budget', store=True),
        'shop_id': fields.related('general_budget_id', 'shop_id', type='many2one', relation='sale.shop',
                                  string='Shop', store=True),
        'department_id': fields.related('general_budget_id', 'department_id', type='many2one', relation='hr.department',
                                        string='Department', store=True),
        'company_id': fields.related('crossovered_budget_id', 'company_id', type='many2one', relation='res.company', string='Company', store=True)
...
class account_analytic_account(osv.osv):
    ...
    _columns = {
        ...
        'type_account': fields.selection([('cost', 'Cost'),
                                          ('income', 'Income')],
                                         'Account Type', select=True),
    }

    _default = {'type_account': 'cost'}
