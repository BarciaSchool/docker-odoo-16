...
class sales_partner(osv.osv):
    ...
    def amount_bonus(self, cr, uid, ids, name, args, context=None):
        ...
        for datas in self.browse(cr, uid, ids, context=context):
            ...
            if partner_id:
                bon_ids = self.pool.get('sales.loyalty.partner.line').search(cr, uid, [('partner_id', '=', partner_id),
                                                                                       ('active', '=', True)])
    ...
    _columns = {'name': fields.char('Código', size=15, help='Un nombre indicativo de la promoción'),
                ...
                'total_invoices': fields.function(amount_bonus, method=True, digits_compute=dp.get_precision('Account'), string='Total Documentos',
                                                  multi='vat_amount', help='El bono que ha acumulado'),
                'received_bonus': fields.function(amount_bonus, method=True, digits_compute=dp.get_precision('Account'), string='Bono acumulado',
                                                  multi='vat_amount', help='El bono que ha acumulado'),
                'redeem_bonus': fields.function(amount_bonus, method=True, digits_compute=dp.get_precision('Account'), string='Bono Redimindo',
                                                multi='vat_amount', help='El valor de bonos que ha redimido'),
                'expired_bonus': fields.function(amount_bonus, method=True, digits_compute=dp.get_precision('Account'), string='Bono Expirado',
                                                 multi='vat_amount', help='El valor de bonos que ha expirado'),
                'actual_bonus': fields.function(amount_bonus, method=True, digits_compute=dp.get_precision('Account'), string='Saldo',
                                                multi='vat_amount', help='El saldo de bonos que puede utilizar', store=True),
                'minimun_purchase': fields.function(amount_bonus, method=True, digits_compute=dp.get_precision('Account'), string='Compra Mínima',
                                                    multi='vat_amount', help='El saldo de bonos que puede utilizar', store=True),
        ...
        if partner_id:
            ...
            if not loyalty_card:
                raise osv.except_osv(_('¡Acción Inválida!'),
                                     _('El cliente no ha realizado movimientos en el período que generen una Tarjeta Promocional.'))
            loyalty_ids = loyalty_obj.search(cr, uid, [('partner_id', '=', partner_id.id), ('date', '>=', date_from), ('date', '<=', date_to),
                                                       ('active', '=', True)])
            if loyalty_ids:
...
class sales_partner_line(osv.osv):
    ...
    _columns = {'type': fields.selection([('add', '+'),
                                          ('subtract', '-'),
                                          ], 'Tipo'),
                ...
                'state': fields.selection([('expired', 'Expirado'),
                                           ('redimed', 'Redimido'),
                                           ('pending', 'Pendiente'),
                                           ('cancel', 'Anulado')
                                           ], 'Estado'),
...
class res_partner(osv.osv):
    ...
    _columns = {'loyalty_card': fields.char('Código', size=15, help='El código del cliente para aplicar su bono'),
                'loyalty_email': fields.boolean('Enviado Correo')}

    _defaults = {'loyalty_email': False}
            ...
            if not partner.loyalty_card:
                for x in range(1, len(partner.vat)):
                    ...
                    if tv and tv[:2] == 'EC':
                        ...
                        if loyalty_card:
                            vals['loyalty_card'] = loyalty_card
                            self.pool.get('sales.loyalty.partner').create(cr, uid, {'name': loyalty_card, 'partner_id': partner.id,
                                                                                    'vat': partner.vat})
            else:
                loyalty_id = self.pool.get('sales.loyalty.partner').search(cr, uid, [('name', '=', loyalty_card)])
                if not loyalty_id:
                    self.pool.get('sales.loyalty.partner').create(cr, uid, {'name': partner.loyalty_card,
                                                                            'partner_id': partner.id, 'vat': partner.vat})
...
class report_loyalty(osv.osv):
    ...
    _defaults = {'company_id': lambda self, cr, uid, c:
                 self.pool.get('res.company')._company_default_get(cr, uid, 'sales.loyalty', context=c),
                 'date_from': '2010-01-01 00:00:00',
                 'date_to': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                 }
            ...
            if il:
                for i in il:
                    ...
                    if i[14] == 'expired':
                        ...
                        estado = 'Anulado'
                    cr.execute("""INSERT INTO reports_loyalty_partner_line(type, mode_id, invoice_id, amount_invoice, date, percent, campaing, bonus,
    pending,state,date_start,date_expired,partner_id,name,wizard_id, create_date, create_uid)
    values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""", (type_d, mode, i[7], i[8], i[3], i[11], i[4], i[9], i[10], estado,
                                                                    i[12], i[13], i[1], i[2], ids[0], date, uid))
            return True
