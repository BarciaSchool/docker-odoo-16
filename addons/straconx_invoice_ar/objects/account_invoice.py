                        ...
                        if taxes:
                            for t in taxes:
                                ...
                                if tax_ids:
                                    ...
                                    if tax_code.ref_base_code_id.tax_type in ('vat', 'perception'):
...
#                         price_unit = float( price_unit)
    ...
    _columns = {
        ...
        'perception_value': fields.float('Percepción', digits_compute=dp.get_precision('AccountInvoice'),),
    ...
    def onchange_offer(self, cr, uid, ids, product=False, offer=0.0, qty=0, price_unit=0.0, margin=0.0, price_product=0.0, discount=0, taxes=False,
            ...
            if taxes:
                ...
                if taxes_id_search:
                    for t in taxes_id_search:
                        ...
                        if tax_code and tax_code.ref_base_code_id.tax_type == 'perception':
                            line_vat = True
                            if tax_code and change_price_product:
                                price_unit = round(price_product * (1 - (discount*0.01))*(1-(offer*.01)), di)
                                price_perc = price_product * (1 + tax_code.amount_variable)
                                amount_perc += round(((price_unit * qty) * tax_code.amount_variable), di)
                            elif tax_code and change_price_net:
                                price_product = price_iva / (1 + tax_code.amount_variable)
                                price_unit = round(price_product * (1 - (discount*0.01))*(1-(offer*.01)), di)
                                amount_perc += round(((price_unit * qty) * tax_code.amount_variable), di)
                            else:
                                amount_perc = 0.0
                            value_vat = tax_code.amount
                    #price_iva += amount_perc
                else:
                    if change_price_net:
                        price_product = price_iva
                    elif change_price_product:
        ...
        result['perception_value'] = round(amount_perc, di)
    ...
    def product_id_change(self, cr, uid, ids, product=False, uom=False, qty=0, name=False, type='out_invoice', partner_id=False,
                          ...
                          discount=0, offer=0, shop_id=False):
        if context is None:
            context = {}
        result = {}
        account_tax_obj = self.pool.get('account.tax')
    ...
    def onchange_account_id(self, cr, uid, ids, product_id, partner_id, inv_type, fposition_id, account_id, name, company_id, shop_id, currency_id):
        ...
        if account_id:
            ...
            if account_ids and account_ids.user_type.code == 'expense':
                if account_ids.analytic_account_id:
                    result['account_analytic_id'] = account_ids.analytic_account_id.id
                else:
                    raise osv.except_osv(_('Error!'), _("La cuenta %s debe tener asignada un cuenta de costo.") % (account_ids.name))
                if cost_journal:
...
class account_invoice(osv.osv):
    ...
    def get_amount_tax_invoice(self, cr, uid, ids, name, args, context=None):
        ...
        for invoice in self.browse(cr, uid, ids, context=context):
            res[invoice.id] = {
                ...
                'amount_base_vat_21': 0.0,
                'amount_base_vat_27': 0.0,
            ...
            if invoice.origin_transaction == 'international' and invoice.journal_id.type in ('in_invoice', 'in_refund'):
                ...
                for line in invoice.invoice_line:
                    if not line.invoice_line_tax_id:
                        res[invoice.id]['amount_base_vat_untaxes'] += round(line.price_subtotal, 3)
                    for tax_id in line.invoice_line_tax_id:
                        tp_tax = tax_id
                        if tp_tax.tax_type == 'vat':
                            if tp_tax.amount == 0:
                                res[invoice.id]['amount_base_vat_00'] += round(line.price_subtotal, 6)
                            elif tp_tax.amount == 0.105:
                                res[invoice.id]['amount_base_vat_12'] += round(line.price_subtotal, 3)
                            elif tp_tax.amount == 0.21:
                                res[invoice.id]['amount_base_vat_21'] += round(line.price_subtotal, 3)
                            elif tp_tax.amount == 0.27:
                                res[invoice.id]['amount_base_vat_27'] += round(line.price_subtotal, 3)
        return res
    ...
    def get_amount_total_invoice(self, cr, uid, ids, name, args, context=None):
        ...
        for invoice in self.browse(cr, uid, ids, context=context):
            ...
            for line in invoice.invoice_line:
                ...
                res[invoice.id]['amount_tax_other'] += round((line.perception_value), di)
                res[invoice.id]['amount_total'] = round(res[invoice.id]['amount_tax_other'] + res[invoice.id]['amount_untaxed'] +
                                                        res[invoice.id]['amount_total_vat'], di)
        return res
    ...
    _columns = {
        ...
        'amount_tax_other': fields.function(_amount_total_invoice, method=True, digits_compute=dp.get_precision('Account'), string='Perception', multi='vat_amount', store=True),
        ...
        'amount_base_vat_27': fields.function(_amount_tax_invoice, method=True, digits_compute=dp.get_precision('Account'), string='BASE 27%', multi='tax_amount', store=True),
        'amount_base_vat_21': fields.function(_amount_tax_invoice, method=True, digits_compute=dp.get_precision('Account'), string='BASE 21%', multi='tax_amount', store=True),
        'amount_base_vat_12': fields.function(_amount_tax_invoice, method=True, digits_compute=dp.get_precision('Account'), string='BASE 10.5%', multi='tax_amount', store=True),
        ...
        #'type_change': fields.float('Tipo de Cambio', digits_compute=dp.get_precision('Account'), states={'open':[('readonly', True)], 'close':[('readonly', True)], 'draft':[('readonly', False)]}),
    ...
    _sql_constraints = [('number_uniq', 'unique(invoice_number, company_id,partner_id,journal_id, type, state)',
                         'Invoice Number must be unique per Company!'),
                        ('number_out_uniq', 'unique(invoice_number_out,invoice_number,partner_id,journal_id, type, company_id, state)',
                         'There is another invoice in the shop and shop with this number, please verify'),
                        ('number_in_uniq', 'unique(invoice_number_in,invoice_number,partner_id,journal_id,type, tax_sustent,state, company_id)',
                         'There is another invoice for this supplier with this number, please verify'),
                        ]

    def onchange_company_id(self, cr, uid, ids, company_id=False, part_id=False, type=False, invoice_line=False, date=False,
        ...
        month_id = self.pool.get('account.period').search(cr, uid, [('date_start', '<=', date),
                                                                    ('date_stop', '>=', date),
...
#                 shop_ids.append(s.shop_id.id)
        ...
        if not statement_id:
            ...
            if printer_point_ids:
                for s in printer_point_ids:
                    s = printer_obj.browse(cr, uid, s)
                    box_ids.append(s.id)
                    if s.shop_id:
                        shop_ids.append(s.shop_id.id)
        if shop:
            ...
            if not res:
                ...
                warning = {'title': _('Error!'),
                           'message': _(('You must be selected in the shop %s the journal respective for this document.') % _(name))}
...
#                 if not invoice.type_change or vals.get('type_change', False) > 0:
#                     raise osv.except_osv(_('¡Error!'), _('Debe realizar la conversión de la moneda %s a %s.')%
#                                          (invoice.currency_id.name, invoice.company_id.currency_id.name))
                    ...
                    for line in inv.invoice_line:
                        if line.account_id.id == inv.account_id.id:
                            raise osv.except_osv(_('¡Aviso!'),
                                                 ...
                                                 (line.name))
            if not inv.partner_id.email and (inv.printer_id.type_printer == 'electronic' or inv.electronic) and type == "out_invoice":
            ...
            if inv.invoice_line:
                ...
                for line in inv.invoice_line:
                    ...
                    cr.execute(sql, (create_date, create_uid, reference, inv.journal_id.id, None, part, ref, date, price_subtotal, 0.00,
                                     line.account_id.id, price_subtotal_currency, inv.currency_id.id, ref, line.quantity, product_id, product_uom,
                                     account_analytic_id, period_id, move_id, state_move, inv.company_id.id, shop_id, department_id, cost_journal))
...
#        cr.commit()
        ...
        for invoice in self.browse(cr, uid, ids, context):
            ...
            if (invoice.type in ('out_invoice', 'out_refund')) or (invoice.type ==
                                                                   'in_invoice' and invoice.journal_id.type ==
                                                                   'purchase_liquidation') or (invoice.type == 'in_refund' and
                ...
                if invoice.migrate:
                    ...
                    if invoice.old_invoice_id:
                        ...
                        self.write(cr, uid, [invoice.id], {'invoice_number': number,
...
#                                                            }, context)
                ...
                vals = []
                if (invoice.origin_transaction == 'local' and not invoice.refund_type=='internal'):
...
#                         raise osv.except_osv(_('Invalid action!'), _('Not exist tax sustent for the document, please check'))
        ...
        if move_id:
            ...
            cr.execute('UPDATE account_analytic_line SET ref=%s FROM account_move_line WHERE account_move_line.move_id = %s '
...
#        return res
            ...
            if i['payment_ids']:
                for move_line in i['payment_ids']:
                    if move_line.reconcile_partial_id and move_line.reconcile_partial_id.line_partial_ids:
                        raise osv.except_osv(_('Error !'), _('No se puede cancelar un factura parcialmente pagada. '
                                                             'Necesita eliminar los pagos realizados'))
            if i.comment:
                comment = i.comment + '\n'
            if i.move_id:
    ...
    def action_cancel_draft(self, cr, uid, ids, *args):
        for invoice in self.browse(cr, uid, ids):
            if invoice.flag:
                line_id = None
            self.write(cr, uid, [invoice.id], {'invoice_number': None,
                                               'cancel_user_id': None,
                                               'cancel_date': None,
    ...
    def clean_values_invoice_cancel(self, cr, uid, ids, context):
        ...
        cr.execute("""UPDATE account_invoice set amount_tax_withhold_vat = 0.0, amount_tax_withhold = 0.0, amount_tax_other = 0.0, amount_untaxed = 0.0,
                        amount_total = 0.0, amount_total_vat = 0.0, residual=0.0, amount_tax = 0.00, amount_base_vat_12 = 0.0
...
#         default.update(self.onchange_cash(cr, uid, ids, invoice.company_id.id, invoice.shop_id.id, invoice.type, invoice.printer_id.id, invoice.journal_id.id, context)['value'])
        ...
        for id in ids:
            for taxe in ait_obj.compute(cr, uid, id, context=ctx).values():
                if taxe:
                    ...
                    create_uid = uid
                    create_date = time.strftime('%Y-%m-%d %H:%M:%S')
                    sql = """INSERT INTO account_invoice_tax(create_uid, create_date,tax_amount, account_id,
                    sequence, company_id, invoice_id, manual, base_amount, base_code_id, amount, base, tax_code_id, name)
                    VALUES (%s,'%s',%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,'%s')""" % (create_uid, create_date,
                                                                                 tax_amount, account_id, sequence,
                                                                                 company_id, invoice_id, manual, base_amount,
    ...
    def onchange_line_ids(self, cr, uid, ids, line_dr_ids, context=None):
        ...
        line_dr_ids = account_voucher.resolve_o2m_operations(cr, uid, line_osv, line_dr_ids, ['price_iva', 'price_subtotal', 'price_product', 'offer',
                                                                                              'discount', 'price_unit', 'iva_value','perception_value' 'quantity',
                                                                                              'invoice_line_tax_id'], context)
        total = 0.00
        amount_tax = 0.00
        amount_tax2 = 0.00
        total_quantity = 0.00
        ...
        for line in line_dr_ids:
            ...
            perception_value = line.get('perception_value', 0.0)
            ...
            amount_tax += iva_value
            amount_tax2 += perception_value
            total += amount
            line_taxes = line.get('invoice_line_tax_id', False)
            if line_taxes:
                if type(line_taxes) is tuple:
                    if line_taxes[0][2]:                
                        tax = line_taxes[0][2]
                elif type(line_taxes) is list:
                    if line_taxes[0][2]:                
                        tax = line_taxes[0][2]
                    else:
                        tax = []
                    ...
                    tax = line_taxes
                for t in tax:
                    tax_id = tax_obj.browse(cr, uid, t)
                    if tax_id.tax_type == 'vat':
                        print "Ok"

        amount_total = total + amount_tax + amount_tax2
        return {'value': {'authorized': b,
                          'pretotal': pretotal,
                          'amount_untaxed': total,
                          'amount_total_vat': amount_tax,
                          'amount_total': amount_total,
                          'amount_total_discount': total_discount,
                          'amount_total_offer': total_offer,
                          'amount_tax_other': amount_tax2,
                          'amount_base_vat_12': 0.0,
                          'amount_base_vat_21': 0.0,
                          'amount_base_vat_27': 0.0,
                          'amount_base_vat_00': 0.0,
                          ...
                          'amount_tax_withhold_vat': 0.0,
                          'amount_tax_withhold': 0.0,
                          'account_quantity': total_quantity}}
