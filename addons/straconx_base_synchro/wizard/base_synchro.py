...
#        pool1 = RPCProxy(server)
        if server:
            database_remote = server.server_db
            user = server.login
            password = server.password
            host = server.server_url
            port = server.server_port
        pool1 = psycopg2.connect(database=database_remote, user=user, password=password, host=host, port=port, options='-c statement_timeout=15s')
        pool2 = pool
        #try:
