...
class account_debit_note(osv.osv):
    ...
    def _amount_residual(self, cr, uid, ids, name, args, context=None):
        ...
        for debit in self.browse(cr, uid, ids, context=context):
            ...
            if debit.move_id:
                for m in debit.move_id.line_id:
                    if debit.type in ('debit_customer', 'advance_supplier'):
                        if m.account_id.type in ('receivable'):
                            if m.reconcile_partial_id:
                                ...
                                residual += m.amount_residual_currency
                    if debit.type in ('debit_supplier', 'advance_customer'):
    ...
    def _get_shop(self, cr, uid, context=None):
        ...
        if type and type in ('debit_customer', 'advance_customer'):
            ...
            if user_id.is_supervisor or user_id.is_warehouse_user:
    ...
    _columns = {'number': fields.char('Number', size=32, required=False, readonly=False),
                "line_ids": fields.one2many('account.debit.note.line', 'debit_note_id', 'Debit Note Line',
                ...
                'residual': fields.function(_amount_residual, method=True, digits_compute=dp.get_precision('Account'), string='Residual',
                                            store={'account.debit.note': (lambda self, cr, uid, ids, c={}: ids, ['line_ids', 'move_id'], 50),
                ...
                'type': fields.selection([('debit_customer', 'Customer Debit Note'),
    ...
    def get_account_period(self, cr, uid, context):
            search_account_period = self.pool.get('account.period').search(cr, uid,
    ...
    def copy(self, cr, uid, ids, default={}, context=None):
        default = default or {}
        debit_id = self.browse(cr, uid, ids, context)
        default.update({
            'move_id': False,
            'number': False,
            'name': False
            })
        return super(account_debit_note, self).copy(cr, uid, ids, default, context)
    ...
    def write(self, cr, uid, ids, vals, context={}):
        ...
        for debit in self.browse(cr, uid, ids, context=context):
            ...
            if debit.move_id.line_id and round(residual, 2) == 0.00 and debit.state == 'posted':
                vals['state'] = 'paid'
            elif debit.move_id.line_id and round(residual, 2) != 0.00 and debit.state == 'paid':
                vals['state'] = 'posted'
    ...
    def onchange_company(self, cr, uid, ids, company=None, type=None, context=None):
        ...
        if company:
            ...
            values['period_id'] = self.pool.get('account.period').search(cr, uid, [('date_start', '<=', time.strftime('%Y-%m-%d')),
                                                                                   ('date_stop', '>=', time.strftime('%Y-%m-%d')),
                                                                                   ('company_id', '=', company)])[0]
        return {'value': values, 'domain': {'shop_id': [('company_id', '=', company)],
                                            'journal_id': [('company_id', '=', company)],
                                            'period_id': [('company_id', '=', company)]}}
    ...
    def onchange_date(self, cr, uid, ids, date, company_id, context=None):
        default = {}
        if date:
            period_ids = self.pool.get('account.period').search(cr, uid, [('date_start', '<=', date or time.strftime('%Y-%m-%d')),
                                                                          ('date_stop', '>=', date or time.strftime('%Y-%m-%d')),
