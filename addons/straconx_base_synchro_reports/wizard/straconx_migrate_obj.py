...
class migrate_objects(osv.osv_memory):
    ...
    _columns = {'migrate_id': fields.text('ID', help="Puede agregar varios id separados por espacios"),
    ...
    def action_migrate(self, cr, uid, ids, context=None):
        ...
        for obj in self.browse(cr, uid, ids, context=context):
            ...
            if obj.shop_id:
                ...
                source_or = psycopg2.connect(database=pool, user=server_login, password=server_password, host=host_pool, port=server_port,
                                             options='-c statement_timeout=60s')
                conection = psycopg2.connect(database=database_dest, user=server_login, password=server_password, host=server_url, port=server_port,
                                             options='-c statement_timeout=60s')
