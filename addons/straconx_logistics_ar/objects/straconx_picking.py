...
#        return invoice_state
    ...
    _columns = {'confirm_reposition': fields.boolean('Confirm Reposition', required=False),
                ...
                'origin': fields.char('Origin', size=128, help="Reference of the document that produced this picking.", select=True, readonly=True,
                                      states={'draft': [('readonly', False)]}),
                'date': fields.date('Order Date', help="Date of Order", select=True, readonly=True, states={'draft': [('readonly', False)]}),
                'date_expected': fields.date('Scheduled Date', readonly=True, states={'draft': [('readonly', False)]}, select=True,
...
#            self.pool.get('stock.picking').action_done(cr,uid,pick.id)
    ...
    def get_prepare_invoice_line(self, cr, uid, group, picking, move_line, invoice_id,
        ...
        if picking.purchase_id.currency_id.id != picking.company_id.currency_id.id:
            cd = picking.company_id.currency_id.rate
        else:
            cd = 1.00
        ...
        discount = (self._get_discount_invoice(cr, uid, move_line)) * cd
        offer = (self._get_offer_invoice(cr, uid, move_line)) * cd
        price_product = (self._get_price_original(cr, uid, move_line)) * cd
        ...
        if taxes:
            ...
            for t in taxes:
                ...
                if tax_code.ref_base_code_id.tax_type == 'perception':
                    if discount == 0.00 and offer == 0.00:
                        amount_percep += float('%.3f' % ((price_unit)))*tax_code.amount_variable
                    else:
                        price_unit = p_net
                        amount_percep += float('%.3f' % ((price_unit)))*tax_code.amount_variable
                        
                amount_iva = amount_iva
                amount_percep = amount_percep
        else:
            amount_iva = 0.00
            amount_percep = 0.00
        ...
        return {
            ...
            'perception_value': float('%.3f' % (amount_percep *qty)) or 0.0,
            'quantity': qty,
