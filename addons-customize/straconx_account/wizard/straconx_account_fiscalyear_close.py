...
class account_fiscalyear_close(osv.osv_memory):
    ...
    def data_save(self, cr, uid, ids, context=None):
        ...
        cr.execute("SELECT id FROM account_period WHERE date_stop < (SELECT date_start FROM account_fiscalyear WHERE id = %s)",
        ...
        shop_ids = self.pool.get('sale.shop').search(cr, uid, [('headquarter', '=', True), ('company_id', '=', period.company_id.id)])
        if shop_ids:
        ...
        vals = {
            ...
            'shop_id': shop_id.id,
            'partner_id': period.company_id.partner_id.id,
            'details': 'ASIENTO DE INICIO DE PERIODO',
            'ref': 'AÑO '+period.fiscalyear_id.name
        }
                     ...
                     WHERE b.account_id IN %s
                       ...
                       AND b.reconcile_id IN (SELECT DISTINCT(reconcile_id)
                                          ...
                                          WHERE a.period_id IN ('''+fy2_period_set+''')))''', (new_journal.id,
                                                                                               period.id, period.date_start,
                                                                                               move_id, tuple(account_ids),))
        ...
        cr.execute('UPDATE account_move_line set active = True where move_id = %s', (move_id,))
        ...
        if ids:
            ...
            #  set the creation date of the reconcilation at the first day of the new fiscalyear,
            #  in order to have good figures in the aged trial balance
