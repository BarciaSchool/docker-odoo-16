...
class product_shops(osv.osv):
    ...
    def do_product_conection(self, cr, uid, ids, context=None):
            ...
            sql="""SELECT pu.location_ubication_id, pu.product_id,
            ...
            for w in self.browse(cr,uid,ids):
                ...
                location_ids = location_obj.search(cr,uid,[('usage','=','internal'),('company_id','=',company_id),('active','=',True),('location_id','<>',None)])
                inputs = 0.00
                sales = 0.00
                transfer_send = 0.00
                transfer_unit = 0.00
          ...
          #                      raise osv.except_osv('¡Error!', _("NO existe conexión con la base de datos %s en la dirección %s.")%(database,host))
                            ...
                            if conection:
                                ...
                                conect.execute(sql,(location, product_id, location, product_id, location, product_id, location, product_id,location, product_id,location, product_id, location, product_id, location, product_id, location, product_id, location, product_id))
                                il = conect.fetchall()                                    
                                if il:
                                    inputs+=il[0][2]
                                    sales +=il[0][3]
                                    transfer_send +=il[0][4]
                                    transfer_unit +=il[0][5]
                                    refunds +=il[0][6]
                                    stock += il[0][7]
                                    inventories += il[0][8]
                                    cr.execute("""INSERT INTO wizard_product_ubication_lines(location_id, product_id,purchase_unit,sales_unit,transfer_send, transfer_unit,refunds,stock_unit,inventories,wizard_id)
                                    values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",(il[0][0],il[0][1],il[0][2],il[0][3],il[0][4],il[0][5],il[0][6],il[0][7],il[0][8],w.id))
                ...
                manufacturer = 0.00                    
                cr.execute("""update wizard_product_ubication set write_date =now(), stock=%s, inputs=%s, outputs=%s, manufacturer=%s, inventory=%s, transfers=%s where id=%s""",(stock,inputs,outputs,manufacturer,inventory,transfers,ids[0]))
                return True
