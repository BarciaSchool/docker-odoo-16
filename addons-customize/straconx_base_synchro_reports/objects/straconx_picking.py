                    ...
                    if shop_ids:
                        ...
                        if shop_id_dest and shop_id_dest.server_db and shop_id_dest.server_url: 
                            ...
                            try:
                                ...
                                for m in pick.move_lines:
                                    if m.product_id:
                                        conect.execute("""update product_template set write_date = now(), standard_price=%s , state ='sellable' where id = (select product_tmpl_id from product_product where id =%s)""",(m.product_id.standard_price, m.product_id.id))
                                conection.commit()
                            except psycopg2.Error: 
                                _logger.exception('Connection to the database failed')
                                raise osv.except_osv('¡Error!', _("NO existe conexión con la base de datos %s en la dirección %s.")%(shop_dest_id_database,shop_dest_id_database))                                                  
                                ...
                                for model_ids in model_objs:
                                    ...
                                    if nuevo_local:
                                        for nuevo in nuevo_local:
                                            ...
                                            rep_update = conect.fetchall()                                            
                                            if old_d == rep_update:
                                                osv._logger.warning('El registro %s del modelo %s ya existe en la base de datos %s',nuevo, model, shop_id_database)    
                                            elif not rep_update:
                                                source.execute(tables)
                                                fields_sri_remote = source.fetchall()
                                                fields_app = ""
                                                ...
                                                try:
                                                    ...
                                                    osv._logger.warning('El registro %s del modelo %s ha tenido problema de creación en la base de datos %s',nuevo, model, shop_id_database)
                                            else:
                                                new_sql = ""
                                                if old_d:
                                                    fields_app = ""                                    
