...
class open_statement(osv.osv_memory):
    ...
    def open_statement(self, cr, uid, ids, context=None):
        ...
        name = context.get('name', _('Cash Register'))
...
#             auth_line = False
#             if not pt_id.type:
#                 auth_line = self.pool.get('sri.authorization.line')
#                 if auth_line:
#                     auth_id = auth_line.search(cr, uid, [('printer_id', '=', printer_id), ('state', '=', True),
#                                                          ('name', 'in', ('sale', 'purchase'))])
#                     if not auth_id:
#                         raise osv.except_osv(_('Message'), _('El punto de impresión que desea abrir no tiene autorizaciones del SRI activas'))
        for journal in journal_obj.browse(cr, uid, journal_ids, context=context):
            ...
            statement_obj.create(cr, uid, data, context=context)
        ...
        return {
            'domain': "[('state', '=', 'draft'),('user_id', '=', " + str(uid) + "), ('journal_id', '=', "
            + str(journal.id) + "),('journal_id.type', '=','" + journal_type+"')]",
            'name': name,
            'view_type': 'form',
            'view_mode': 'form,tree',
