...
class deposit_register(osv.osv):
    ...
    def action_confirmed(self,cr,uid,ids,context={}):
        for dp in self.browse(cr,uid,ids):            
            dp_ids = self.search(cr, uid, [('receipt','=',dp.receipt),('account_deposit_id','=',dp.account_deposit_id.id)])
            if len(dp_ids) > 1:
               raise osv.except_osv(_("Error"),_("No se puede realizar el depósito porque ya existe una papeleta con el mismo número."))
