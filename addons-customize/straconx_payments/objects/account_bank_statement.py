...
#                    res2[statement.id]['total_outlet_encoding'] += line.amount
    ...
    def _get_default_shop(self, cr, uid, context=None):
        if self.pool.get('res.users').browse(cr, uid, uid, context).cash_box_default_id:
            ...
            if printer_id:
                printer_id = self.pool.get('printer.point').browse(cr, uid, printer_id, context)
                if printer_id and printer_id.shop_id:
                    return printer_id.shop_id.id
        return None
...
#        return self.write(cr, uid, ids, {'state':'confirm'}, context=context)
    ...
    def button_confirm_cash(self, cr, uid, ids, context=None):
        ...
        for cash in self.browse(cr, uid, ids, context):
            for line in cash.line_ids:
                if line.voucher_id:
                    inv = line.voucher_id.invoice_id
                elif line.payment_id:
                    inv = line.payment_id.invoice_id
                if inv and inv.pos and inv.state == 'open':
                    raise osv.except_osv(_('Error !'),
                            _('No se pueden cerrar cajas que contengan facturas POS en estado ABIERTO.'))
            date= time.strftime("%Y-%m-%d %H:%M:%S")
            open_date = cash.date
            ...
            if round(cash.cash_diferences,2) >0:
                self.send_mail_cash(cr, uid, ids, context=None)
#            self.write(cr, uid, [cash.id], {'closing_date': date, 'original_closing_date':original_closing_date}, context=context)
        return True
    
    def send_mail_cash(self, cr, uid, ids, context=None):
        todo = [] 
        xml_id = 'email_cash_box_confirm'
        mod_obj = self.pool.get('ir.model.data')
        for cc in self.browse(cr, uid, ids, context=context):
            if not cc.name:
                name = self.pool.get('ir.sequence').get(cr, uid, 'account.bank.statement')
            else:
                name = cc.name                          
            template_ids = mod_obj.get_object_reference(cr, uid, 'straconx_payments', xml_id)
 #           if not cc.company_id.cash_user.user_email:
 #               raise osv.except_osv(_('Error!'), _('No existe un email para el envío del correo electrónico para el cierre de caja.'))            
            if not template_ids:
                raise osv.except_osv(_('Error!'), _('No existe una plantilla para el envío del correo electrónico para el cierre de caja.'))            
            else:
                template_id = template_ids[1]
            self.generate_email_confirm(cr,uid,template_id,cc.id,context)            
        return True
    
    def generate_email_confirm(self, cr, uid, template_id, res_id, context=None):
        if context is None:
            context = {}
      ...
      #  ir_attachment = self.pool.get('ir.attachment')
        ...
        cash_id = cash_obj.browse(cr,uid,res_id)
...
#        cash_diferences = bank_obj.balance_end - total - bank_obj.balance_start
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        for t in stat:
            if t['state'] in ('draft'):
                ...
                raise osv.except_osv(_('Invalid action !'), _('In order to delete a bank statement, you must first cancel it to delete lated journal items.'))

            cr.execute("""update account_bank_statement_line set write_date =now(),  active=False, amount = 0.00 where statement_id in %s """,
                       (tuple(ids),))
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        for rs in res:
            super(account_bank_statement, self).write(cr, uid, [rs], res.get(rs))
...
class res_company(osv.osv):
    ...
    _columns = {
                'cash_user':fields.many2one('res.users', 'Financiero', readonly=False),
                }
res_company()
