    ...
    def proforma_voucher(self, cr, uid, ids, context=None):
        ...
        for voucher in self.browse(cr, uid, ids, context=context):            
            ...
            for line in voucher.payments:
                if voucher.date != line.deposit_date:
                   raise osv.except_osv(_('Error!'), _('La fecha del pago debe ser igual a la fecha de la cabecera.'))
...
#                if cash_obj.browse(cr, uid, cash_id[0], context).printer_id.shop_id.point_of_sale:
    ...
    def first_move_line_get(self, cr, uid, voucher_id, move_id, company_currency, current_currency, pay, context=None):
        ...
        if pay.mode_id.check or pay.mode_id.credit_card:
            ...
            if pay.deposit_date != voucher_brw.date:
                date = pay.deposit_date
                date_m = pay.deposit_date
            else:
                date = voucher_brw.date
                date_m = voucher_brw.date
...
#                 raise osv.except_osv(_('Error !'), _('the sum of the payments mode is incorrect with the amount, please check!'))
            ...
            for line in voucher.payments:
                if voucher.date != line.deposit_date:
                    move_pool.write(cr, uid, [move_id], {'date':line.deposit_date})
                    voucher.write({'date':line.deposit_date})
    ...
    def voucher_move_line_create(self, cr, uid, voucher_id, line_total, move_id, company_currency, current_currency, context=None):
        ...
        for line in voucher_brw.line_ids:
            ...
            if line.amount_unreconciled < line.amount:
                raise osv.except_osv(_('!Aviso!'),_("El valor a pagar es mayor que el saldo de la factura. Por favor verifique."))
