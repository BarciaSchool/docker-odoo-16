...
class hr_action_reason_rule(osv.osv):
    ...
    _columns = {
        ...
        'seq': fields.integer('Priority', help='Low values have'
                             ' more priority'),
    ...
    def compute(self, cr, uid, ids, att_ids, context=None):
        ...
        for att in att_pool.browse(cr,uid,att_ids,context=context):
            ...
            if action:
                ...
                if action.id not in res: res[action.id] = []
                res[action.id].append(att.id)
