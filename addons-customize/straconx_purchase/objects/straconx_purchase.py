        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            for line in order.order_line:
                for ab in line.taxes_id:
                    ...
                    if ab.tax_code_id.tax_type in ('perception','perception_iva'):
...
#                 values['attachment_ids'] = att_ids
    ...
    def unlink(self, cr, uid, ids, context=None):
        res_users = 'Orden de Compra anulada por usuario ' + self.pool.get('res.users').browse(cr,uid,uid).login
        for oc in self.browse(cr, uid, ids, context):
            if not oc.state == 'draft':
                raise osv.except_osv(_('Invalid action!'), _('Solo puede inactivar órdenes en estado de presupuesto'))
        cr.execute("""update purchase_order_line set write_date =now(), state='cancel'where order_id in %s """,(tuple(ids),))            
        cr.execute("""update purchase_order set write_date =now(), name=Null, state='cancel', notes=%s where id in %s """,(res_users, tuple(ids)))
        return True
