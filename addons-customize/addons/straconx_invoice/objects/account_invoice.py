...
#                         price_unit = float( price_unit)
    ...
    def get_calculate_discount(self, cr, uid, ids, field_names, arg, context=None):
        ...
        for line in self.browse(cr, uid, ids):
            res[line.id] = {
              'offer_value_total':round((line.price_product * (1 - line.discount / 100)) * line.offer / 100 * line.quantity, 2),
...
#        'uos_id':False,
    ...
    def onchange_read(self, cr, uid, ids, discount_read=0.00,price_unit_read=0.00,price_subtotal_read=0.00,discount=0.00,price_unit=0.00,price_subtotal=0.00,context=None):
        result = {}
    ...
    def onchange_offer(self, cr, uid, ids, product=False, offer=0.0, qty=0, price_unit=0.0, margin=0.0, price_product=0.0, discount=0, taxes=False, price_iva=0.0, fiscal_position=False, context=False):
        warning = {}
        permit_offer = True
        ...
        if inv_type in ('out_invoice', 'out_refund') :
            if offer <= percent_minum and (discount<= discount_product) and authorized: 
                ...
                if not warning:
                    authorized = False
                    warning = {'title': _('¡Requiere Authorización !'),'message': _(('Se ha realizado un cambio de precio o el descuento/oferta es mayor al autorizado a su usuario. Por favor, solicite que su Supervisor apruebe la transacción ingresado la autorización correspondiente.'))}                
...
#        price_subtotal = round((price_unit * qty),di)
    ...
    def product_id_change(self, cr, uid, ids, product=False, uom=False, qty=0, name=False, type='out_invoice', partner_id=False, fposition_id=False, price_unit=0, address_invoice_id=False, currency_id=False, context=False, company_id=False, discount=0, offer=0, shop_id=False):
        if context is None:
            context = {}
        result = {}
        account_tax_obj = self.pool.get('account.tax')
...
#         if shop_id:
#             cr.execute("""select project_id from sale_shop where id=%s""",(shop_id,))
#             analytic = cr.fetchall()
#         if analytic:
#             analytic = analytic[0][0]            
#         else:
#             analytic = company_id.property_analytic_account.id
# 
#         if not analytic:
#             raise osv.except_osv(_('Analytic Account Required!'), _('Need a analytic account for continue. Please configure at shop or company.'))                
        ...
        if product:
            ...
            cr.execute("""select default_code, pt.name, pt.categ_id, pt.uom_id, pp.discount_percent, pt.list_price, pp.p_net, pt.type, 
            ...
            if type in ('out_invoice', 'out_refund'):
                ...
                if not a:
                    ...
                    if len(property_account_income)>0:
                        a = int(property_account_income[0][0].split(',')[1])
                    if not a:
                        cr.execute("""select value_reference from ir_property where name ='property_account_income' and company_id =%s""",(company_id.id,))
                        property_account_income = cr.fetchall()
                        if len(property_account_income)>0:
                            a = int(property_account_income[0][0].split(',')[1])
            else:
                    ...
                    if not a:
                        ...
                        if len(property_account_expense)>0:
                            a = int(property_account_expense[0][0].split(',')[1])    
                        if not a:
                            cr.execute("""select value_reference from ir_property where name ='property_account_expense' and company_id =%s""",(company_id.id,))
                            property_account_expense = cr.fetchall()
                            if len(property_account_expense)>0:
                                a = int(property_account_expense[0][0].split(',')[1])
            ...
            if not segmento_id:
                ...
                if segmento_ids: 
                    ...
                    raise osv.except_osv(_('Acción Inválida!'), _('Necesita especificar por lo menos un segmento predeterminado o asignar un segmento al cliente'))    
                ...
                for s in res.seller_ids:
                    if s.name.id == partner_id:
                        ...
                        if qty < temp_qty:  # If the supplier quantity is greater than entered from user, set minimal.
                            qty = temp_qty
                            result.update({'warning': {'title': _('Warning'), 'message': _('The selected supplier has a minimal quantity set to %s, you cannot purchase less.') % qty}})
            ...
            if type in ('out_invoice', 'out_refund'):
                cr.execute("""select tax_id from product_taxes_rel where prod_id = (select product_tmpl_id from product_product where id=%s)""",(res.id,))
                tax_imp = []
                tax_ids = cr.fetchall()
                if tax_ids:
                    for t in tax_ids:
                        cr.execute("""select id from account_tax where id=%s and company_id=%s""",(t[0], company_id.id))
                        p = cr.fetchone()
                        if p and p[0]: 
                            tax_imp.append(t[0])
                        else:
                            t = self.pool.get('account.tax').browse(cr,uid,t[0])
                            cr.execute("""select id from account_tax where description=%s and company_id=%s and type_tax_use=%s""",(t.description, company_id.id,t.type_tax_use))
                            new_t = cr.fetchone()
                ...
                cr.execute("""select tax_id from product_supplier_taxes_rel where prod_id =(select product_tmpl_id from product_product where id=%s)""",(res.id,))
                tax_imp = []
                tax_ids = cr.fetchall()
                if tax_ids:
                    for t in tax_ids:
            ...
            if uom:
                ...
                if categ_uom_id:
                    pt_uom = product_obj.browse(cr,uid,product).uom_id.id
...
#         product_change_result = self.product_id_change(cr, uid, ids, product_id, False, name=name, type=inv_type, partner_id=partner_id, fposition_id=fposition_id, company_id=company_id, shop_id=shop_id, currency_id=currency_id,context=context)
#         unique_tax_ids = set(tax_ids)
#         if product_change_result and 'value' in product_change_result and 'invoice_line_tax_id' in product_change_result['value']:
#             unique_tax_ids |= set(product_change_result['value']['invoice_line_tax_id'])
#             v['invoice_line_tax_id']=list(unique_tax_ids)
        analytic_account_id = False
        if account_id:
            analytic_id = acc_acc_obj.browse(cr,uid,account_id)
            if analytic_id:
                aa_id = analytic_id.analytic_account_id.id
                analytic_account_id =self.pool.get('account.analytic.account').browse(cr,uid,aa_id)
        v['analytic_account_id'] = analytic_account_id.id
        return {'value': v}
#        return {'value':{'invoice_line_tax_id': list(unique_tax_ids)}}
                    
    def uos_id_change(self, cr, uid, ids, product, uom, qty=0, name=False, type='out_invoice', partner_id=False, fposition_id=False, price_unit=0, address_invoice_id=False, currency_id=False, context=None, company_id=False, shop_id=False):
        if context is None:
            context = {}
        ...
        warning = {}
        res = self.product_id_change(cr, uid, ids, product, uom, qty, name, type, partner_id, fposition_id, price_unit, address_invoice_id, currency_id, context=context, company_id=company_id, shop_id=shop_id)
        if res:
...
class account_invoice(osv.osv):
    ...
    def get_amount_tax_invoice(self, cr, uid, ids, name, args, context=None):
        ...
        for invoice in self.browse(cr, uid, ids, context=context):
            ...
            if invoice.origin_transaction == 'international' and invoice.journal_id.type in ('in_invoice','in_refund'):
                ...
                for line in invoice.invoice_line:
                    if line.iva_value==0:
                            res[invoice.id]['amount_base_vat_00'] += round(line.price_subtotal,3)
                    else:
                        res[invoice.id]['amount_base_vat_12'] += round(line.price_subtotal,3)                  
            res[invoice.id]['amount_base_vat_untaxes'] = invoice.amount_untaxed - res[invoice.id]['amount_base_vat_12'] - res[invoice.id]['amount_base_vat_00']
        return res
    ...
    _sql_constraints = [                        
                        ...
                        ('number_out_uniq', 'unique(invoice_number_out,invoice_number,partner_id,journal_id, company_id)', 'There is another invoice in the shop and shop with this number, please verify'),
                        ('number_in_uniq', 'unique(invoice_number_in,invoice_number,partner_id,journal_id,tax_sustent,state, company_id)', 'There is another invoice for this supplier with this number, please verify'),
                        ]
...
#             result['authorization'] = None
    ...
    def onchange_partner_id(self, cr, uid, ids, type, partner_id,electronic=False,printer=False, date_invoice=False, payment_term=False, partner_bank_id=False, company_id=False, context=None):
...
#             if vals.get('date_invoice2',False):
#                 h = vals['date_invoice2'][11:22]
#                 vals['date_invoice2']= vals['date_invoice'] +' '+h
#             else:
#                 vals['date_invoice2']= vals['date_invoice'] +' '+time.strftime('%H:%M:%S')
            ...
            if invoice.move_id and invoice.state not in ('draft','cancel','proforma','proforma1'):
                ...
                if invoice.move_id:
                    if invoice.move_id.state == 'posted' and reconciled and residual == 0.00:
                        cr.execute("""update account_invoice set write_date = now(), residual = %s, state='paid', reconciled = True where id=%s""",(residual, invoice.id))
                    else:
                        cr.execute("""update account_invoice set write_date = now(), residual = %s , state='open', reconciled = False where id=%s""",(residual, invoice.id))
                else:
                    raise osv.except_osv(_('¡Error!'), _('El movimiento contable de la factura %s con número %s no existe.')%(invoice.id, invoice.invoice_number))
                    #cr.execute("""update account_invoice set write_date = now(), residual = %s where id=%s""",(residual, invoice.id)) 
        return super(account_invoice, self).write(cr, uid, ids, vals, context)

    def create(self, cr, uid, vals, context={}):
#         if vals.get('date_invoice',False):
#             vals['date_invoice2']= vals['date_invoice'][:10] +' ' +time.strftime('%H:%M:%S')        
        if vals.get('authorization_sales' , False):
            vals['authorization'] = self.pool.get('sri.authorization').browse(cr, uid, vals['authorization_sales'], context).name
        if vals.get('authorization_purchase' , False):
            vals['authorization'] = self.pool.get('sri.authorization').browse(cr, uid, vals['authorization_purchase'], context).name
...
#            total, total_currency, iml = self.compute_invoice_totals(cr, uid, inv, company_currency, ref, iml)
            ...
            if inv.journal_id.type == 'purchase_liquidation':
                ...
                name = 'NOTA DE CRÉDITO DE CLIENTES'
                ref = inv.invoice_number_out
            else:
...
#                        price_subtotal = round(round(line.price_product,2) * round(line.quantity,2),2)
                        if len(inv.invoice_line) == 1:
                            price_subtotal = round(line.price_subtotal,di)
                            iva_value = round(line.iva_value,di)
                        else:
                    ...
                    cr.execute(sql, (create_date, create_uid,reference, inv.journal_id.id, None, part, ref, date, price_subtotal, 0.00, line.account_id.id, price_subtotal_currency, inv.currency_id.id, ref, line.quantity, product_id, product_uom, account_analytic_id, period_id, move_id, state_move, inv.company_id.id,shop_id, department_id, cost_journal))                                        
            ...
            if inv.type in ('out_invoice', 'out_refund'):
                ...
                if compute_taxes:
                    for t in compute_taxes:
                        ...
                        if amount_tax2>0 and t_obj.tax_code_id.tax_type == 'vat' and t_obj.amount >0.00:
                            if inv.type in ('in_invoice', 'out_refund'):                    
                                cr.execute(sql, (create_date, create_uid,reference, inv.journal_id.id, None, part, name_tax, date, amount_tax2, 0.00, acc_id_tax, amount_currency_tax2, inv.currency_id.id, name_tax, 1, None, None, account_analytic_id, period_id, move_id, state_move, inv.company_id.id, shop_id, None, None))
                            else:
                                cr.execute(sql, (create_date, create_uid,reference, inv.journal_id.id, None, part, name_tax, date, amount_tax2, 0.00, acc_id_tax, amount_currency_tax2, inv.currency_id.id, name_tax, 1, None, None, account_analytic_id, period_id, move_id, state_move, inv.company_id.id, shop_id, None, None))
                ...
                if compute_taxes:
                    for t in compute_taxes:
                        ...
                        if amount_tax2>0 and t_obj.tax_code_id.tax_type == 'vat' and t_obj.amount >0.00:
                            if inv.type in ('in_invoice', 'out_refund'):                    
                                cr.execute(sql, (create_date, create_uid,reference, inv.journal_id.id, None, part, name_tax, date, amount_tax, 0.00, acc_id_tax, amount_currency_tax, inv.currency_id.id, name_tax, 1, None, None, account_analytic_id, period_id, move_id, state_move, inv.company_id.id,shop_id, None, None))
                            else:
                                cr.execute(sql, (create_date, create_uid,reference, inv.journal_id.id, None, part, name_tax, date, amount_tax2, 0.00, acc_id_tax, amount_currency_tax2, inv.currency_id.id, name_tax, 1, None, None, account_analytic_id, period_id, move_id, state_move, inv.company_id.id,shop_id, None, None))
...
#                 else: 
            ...
            if totlines:
                ...
                for t in totlines:
                    ...
                    if inv.type in ('in_invoice', 'in_refund'):
                        amount = amount
                        cr.execute(sql, (create_date, create_uid,reference, inv.journal_id.id, t[0], part, name, date, 0.00, amount, acc_id, amount_currency, inv.currency_id.id, name, 1, None, None, account_analytic_id, period_id, move_id, state_move, inv.company_id.id,shop_id, None, None))
                    else:
                        amount = amount
                        cr.execute(sql, (create_date, create_uid,reference, inv.journal_id.id, t[0], part, name, date, 0.00, amount, acc_id, amount_currency, inv.currency_id.id, name, 1, None, None, account_analytic_id, period_id, move_id, state_move, inv.company_id.id,shop_id, None, None))                    
            else:
                cr.execute(sql, (create_date, create_uid,reference, inv.journal_id.id, inv.date_due, part, name, date, 0.00, total, acc_id, amount_currency, inv.currency_id.id, name, 1, None, None, account_analytic_id, period_id, move_id, state_move, inv.company_id.id,shop_id, None, None))                
#             discount = inv.amount_total_offer + inv.amount_total_discount
#             discount = float('%.3f' % (discount))
#             if discount > 0.0 and inv.type == 'out_invoice':
#                 acc_dis_id = inv.company_id.property_discount_sales.id
#                 discount_currency = round(discount * cd,di)
#                 if not acc_dis_id:
#                     raise osv.except_osv(_('Error !'), _('You must configure the discount account in the company'))
#                 else:
#                     cr.execute(sql, (reference, inv.journal_id.id, None, part, name, date, 0.00, discount, acc_dis_id, discount_currency, inv.currency_id.id, name, 1, None, None, account_analytic_id, period_id, move_id, state_move, inv.company_id.id))
#             else:
#                 acc_dis_id = inv.company_id.property_discount_sales.id
                ...
                if abs(amount_new) >0:
                    if debit > credit:                    
                        ...
                        if inv.type in ('out_invoice','in_refund'):
                            ...
                            cr.execute(sql, (create_date, create_uid,reference, inv.journal_id.id, None, part, name, date,amount_new, 0.00, acc_dis_id, discount_currency, inv.currency_id.id, name, 1, None, None, account_analytic_id, period_id, move_id, state_move, inv.company_id.id,shop_id,None, None))                 
                        else:
                            discount_currency = round(amount_new * cd,di)   
                            name = 'AJUSTE POR REDONDEO'
                            cr.execute(sql, (create_date, create_uid,reference, inv.journal_id.id, None, part, name, date, 0.00,amount_new, acc_dis_id, discount_currency, inv.currency_id.id, name, 1, None, None, account_analytic_id, period_id, move_id, state_move, inv.company_id.id,shop_id,None, None))                 
                    elif debit < credit:
                        if inv.type in ('in_invoice','out_refund'):                        
