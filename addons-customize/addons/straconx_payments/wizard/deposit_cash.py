...
class deposit_statement_wizard(osv.osv_memory):
    ...
    _columns = {
                'statement_id':fields.many2one('account.bank.statement', 'Cash Registrer', required=False),
    ...
    def action_deposit(self, cr, uid, ids, context=None):
        ...
        for inv in objs:
            ...
            cheques_ids = payments_obj.search(cr,uid,[('vouch_id','in',vouch_ids),('type','=','receipt'),('mode_id.check','=',True),('mode_id.to_deposit','=',True)])
            if cheques_ids:
            ...
            if not obj.line_ids:
                ...
                for total in obj.line_ids:
                    ...
                    move_line = {
                        'name': 'Depósito de '+ total.mode_id.name +' '+total.receipt,
                        'reference':ref_move,
                        'debit': total.amount,
                        'credit': 0,
                    ...
                    self.pool.get('deposit.register').create(cr, uid, {
                                                                       'printer_id':inv.printer_id.id,
                ...
                move_line = {
                            'name': 'Depósito de '+ total.mode_id.name+' '+inv.name,
                            'reference':ref_move,
                            'debit': 0,
                            'credit': suma,
            ...
            if not moves:
                ...
                statement_obj.write(cr, uid, [inv.id,], {'move_id':move_id,})
            for rec_ids in rec_list_ids:
                if len(rec_ids) >= 2:
                    move_line_pool.reconcile_partial(cr, uid, rec_ids)
            statement_obj.write(cr, uid, [inv.id,], {'deposit':True})
...
class deposit_statement_wizard_line(osv.osv_memory):
    ...
    _columns = {
                'wizard_id':fields.many2one('deposit.statement.wizard', 'wizard', required=False),
    ...
    def onchange_mode_id(self, cr, uid, ids, statement, mode_id, context=None):
        ...
        if statement:
            ...
            if not mode_id:
                ...
                modes_ids = self.pool.get('account.total.line').search(cr,uid,[('mode_id','=',mode_id),('cash_id','=',statement_browse.id)])
                if modes_ids:
