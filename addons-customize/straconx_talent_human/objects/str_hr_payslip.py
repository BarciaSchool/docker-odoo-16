...
#                             nb_of_days = (holiday_to - day_from).days
                    ...
                    leave_line = {
                         ...
                         'number_total_days': first_quarter_day + second_quarter_day,
    ...
    def get_inputs(self, cr, uid, contract_ids, date_from, date_to, payslip_id, context=None):
        res = []
        income_lines = self.pool.get('hr.incomes.lines')
        incomes = income_lines.search(cr, uid, [('contract_id','=',contract_ids[0]),('input_id.state','in',('approve','paid')),('state','=','approve'),('input_id.date_from','<=',date_to)]) 
        for inc in incomes:
            i = income_lines.browse(cr,uid,inc,context) 
...
#                         'journal_id': contract_ids and contract_obj.read(cr, uid, contract_ids[0], ['journal_id'], context=context)['journal_id'][0] or False,
    ...
    def compute_sheet(self, cr, uid, ids, context=None):
        old_lines_ids = []
        lines_obj = self.pool.get('hr.payslip.line')
        for payslip in self.browse(cr, uid, ids, context):
            ...
            old_lines_ids = ids and lines_obj.search(cr, uid, [('slip_id', '=', ids[0])], context=context) or False
            if old_lines_ids:
                cr.execute("""delete from hr_payslip_line where slip_id = %s""",(ids[0],))
...
#        discounts_obj = DiscountLine(self.pool, cr, uid, payslip.employee_id.id, discounts)
        ...
        for contract in self.pool.get('hr.contract').browse(cr, uid, contract_ids, context=context):
            ...
            for rule in obj_rule.browse(cr, uid, sorted_rule_ids, context=context):
                ...
                if rule.generate_move and not rule.account_debit and not rule.account_credit:
                    raise osv.except_osv(_('Error'), _('Debe definir las cuentas de débito y crédito para la Regla Salarial %s')% (rule.name))
...
#                 raise osv.except_osv(_('Warning'), _('You can define a analytic account in shop for generate salary moves'))
            ...
            context.update({'search_shop':True})
                ...
                for discount in slip.discount_lines_ids: 
                    ...
                    if discount and discount.amount>0:   
                        ...
                        if account_debit:
                            ...
                            credit_line=self.create_move_line_payslip(cr, uid, None, slip.employee_id.partner_id.id, discount_name or '/', slip.date_to, account_credit, journal_id, period_id, discount.amount, move_ids, slip.number, company_id,l_discount_id, analytic_account_id, shop_id ,department_id,cost_journal,context)
                        ...
                        if discount.discount_line_id.discount_id.name.code == 'SOLIDARIDAD':
                            self.pool.get('hr.discount').create_move_discount(cr, uid,[discount.discount_line_id.discount_id.id],discount.discount_line_id,{})
            for d in accounts_ids.keys():        
                name_account = self.pool.get('account.account').browse(cr,uid,d).name
                l_discount_id = False
                if accounts_ids[d] > 0:
                    context.update({'debit':True})
                    context.update({'credit':False})
                    debit_line=self.create_move_line_payslip(cr, uid, None, slip.employee_id.partner_id.id, name_account, slip.date_to, d, journal_id, period_id, accounts_ids[d], move_ids, slip.number, company_id, l_discount_id, analytic_account_id, shop_id, department_id,cost_journal,context)
                    line_ids.append(debit_line)
                    debit_sum += debit_line['debit'] - debit_line['credit']
                if accounts_ids[d] < 0:
                    context.update({'debit':True})
                    context.update({'credit':False})                 
                    credit_line=self.create_move_line_payslip(cr, uid, None, slip.employee_id.partner_id.id, name_account, slip.date_to, d, journal_id, period_id, accounts_ids[d], move_ids, slip.number, company_id, l_discount_id,analytic_account_id, shop_id, department_id,cost_journal,context)
                    line_ids.append(credit_line)
            ...
            if slip.input_line_ids:
                for income in slip.input_line_ids:
                    if income.i_id:                        
                        if income.i_id.input_id.collection_form == 'middle_end_month' and income.i_id.payslip_id:                                                        
                            income.i_id.write({'payslip_id2':slip.id,'state':'paid'})
                        elif income.i_id.input_id.collection_form == 'middle_end_month' and not income.i_id.payslip_id:
                            income.i_id.write({'payslip_id':slip.id,'state':'approve'})
                        else:
            ...
            context.update({'fy_closing':True})
