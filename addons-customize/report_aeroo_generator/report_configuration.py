...
class report_configuration(osv.osv):
    ...
    _columns = {
        ...
        'type': fields.selection([('account.invoice', 'Invoice'),
                                  ...
                                  ('product.catalog', 'Product Catalog')], 'Type', required=True),
        ...
        'stock_picking_journal_ids': fields.many2many('stock.journal', 'report_configuration_stock_journal_rel',
                                        'report_configuration_id', 'journal_id', 'Journals'),
        'stock_picking_split_invoice': fields.boolean('Split Picking', help='If true, when processing the picking, if it contains more than the specified number of lines, new pickings will be generated.'),
        ...
        # product.catalog
        'item_name' : fields.char('Item Name', size=200, translate=True),
    }
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if isinstance(ids, list):
            self.generate_report(cr, uid, ids, context=context)
        else:
            self.generate_report(cr, uid, [ids], context=context)
        return ids
    ...
    def generate_report(self, cr, uid, ids, context=None):
        ...
        for report_conf in self.browse(cr, uid, ids, context=context):
            ...
            if report_conf.type == 'account.invoice':
                ...
                vals['report_name'] = report_name
            elif report_conf.type == 'stock.picking':
                ...
                vals['report_name'] = report_name
            elif report_conf.type == 'product.catalog':
                vals['model'] = 'product.product'
                report_name = 'product_product_report'
                vals['report_name'] = report_name
